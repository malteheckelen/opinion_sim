...
rc_sh_modelInit <- function(sim) {
  ...
  sim$chosen_actions_temp <- copy(sim$actions_overall)[ actions == best_action , .(agent_id, best_action) ] 
  sim$chosen_actions <- merge(sim$chosen_actions_temp, sim$chosen_actions, by=c("agent_id"), all.x=TRUE, all.y=TRUE) 
  ...
  sim$discourse_memory <- unique(copy(sim$messages)[ , -c("receiver", "assumption_receiver") ]) 
  ...
  sim$discourse_memory <- sim$discourse_memory[ !duplicated(sim$discourse_memory[ , as.logical(lapply(sim$discourse_memory, function(x) !is.list(x) ) ) , with = FALSE ]) , ]
   ...
   if( length(sim$actions_overall[ best_action %in% c("Both", "Send") , best_action ] > 0 ) ) {
    ...
    sim$actions_send_temp <- copy(sim$messages) 
    sim$actions_send_temp <- sim$actions_send_temp[unique(sim$actions_overall[ , .(agent_id, best_action) ] ), on = c("sender" = "agent_id"), nomatch = 0L ] 
    ...
    sim$actions_send[ actions == "Unoptimized" , util_score := 0 - distance_to_past_opinions - distance_message_opinion - distance_message_assumption - msg_compl + involvement] 
    sim$actions_send[ actions == "Unoptimized_appeal", util_score := 0 - distance_to_past_opinions - distance_message_opinion - distance_message_assumption + msg_compl - involvement] 
    sim$actions_send[ actions == "Optimized", util_score := 0 - distance_to_past_opinions - distance_message_opinion - distance_message_assumption - msg_compl + involvement] 
    sim$actions_send[ actions == "Optimized_appeal", util_score := 0 - distance_to_past_opinions - distance_message_opinion - distance_message_assumption + msg_compl - involvement] 
    ...
    sim$chosen_actions_temp <- copy(sim$actions_send)[ actions == best_action , .(agent_id, best_action) ] 
    sim$chosen_actions <- merge(sim$chosen_actions_temp, sim$chosen_actions, by="agent_id", all.x=TRUE, all.y=TRUE) 
    ...
    sim$discourse_memory_temp <- copy(sim$actions_send)[ , -c("util_score")] 
    ...
    sim$discourse_memory_temp <- sim$discourse_memory_temp[copy(sim$messages), on = c("agent_id" = "sender"), nomatch = 0L] 
    ...
    sim$messages <- copy(sim$messages)[sim$actions_send[ best_action == actions , .(agent_id, actions, best_action)], nomatch=0L, on=c("sender" = "agent_id")]  
    ...
    sim$messages <- sim$messages[ !duplicated(sim$messages[ , as.logical(lapply(sim$messages, function(x) !is.list(x) ) ) , with = FALSE ]) , ]
    ...
    sim$messages[ , -c("past_msg_compls", "past_op_compls", "actions", "best_action")] 
    ...
    sim$actions_receive_temp <- copy(sim$messages)[, agent_id := receiver][ , .(agent_id, msg_compl, op_compl_receiver)] 
    sim$actions_receive_temp <- unique(sim$actions_receive_temp) 
    sim$actions_receive_temp <- sim$actions_receive_temp[copy(sim$actions_overall)[ best_action == actions ][ (best_action == "Both" | best_action == "Receive") ][ , .(agent_id)], on = c("agent_id"), nomatch = 0L ] 
    ...
    sim$actions_receive[ actions == "Systematic" , util_score := involvement , by = agent_id ] 
    sim$actions_receive[ actions == "Heuristic" , util_score := complexity , by = agent_id ] 
    ...
    if (nrow(sim$actions_receive[ best_action == "Heuristic" ]) > 0) {
	    ...
	    sim$opinion_updating_h <- sim$opinion_updating_h[sim$actions_receive[ best_action == actions ][ best_action == "Heuristic"]
, on = c("receiver" = "agent_id"), nomatch = 0L ] 
	    sim$opinion_updating_h[ , .(sender, receiver, opinion_receiver, assumption_sender, opt_message, op_compl_receiver, msg_compl, actions, best_action)] 
	    ...
	    sim$opinion_updating_h <- sim$opinion_updating_h[copy(sim$agent_characteristics)[ , .(agent_id, group, expert) ], on=c("sender"="agent_id"), nomatch=0L] 
 ...
 merge_receiver_opinions <- merge_receiver_opinions[ !duplicated(merge_receiver_opinions[ , as.logical(lapply(merge_receiver_opinions, function(x) !is.list(x) ) ) , with = FALSE ]) , ]
 merge_receiver_opinions[ , past_opinions := mapply(function(x) list(eval(parse(text = x))), x=past_opinions   ) ]
	    ...
	    sim$opinion_updating_s <- sim$opinion_updating_s[copy(sim$actions_receive)[ best_action == actions ][ best_action == "Systematic"], on = c("receiver" = "agent_id"), nomatch = 0L ] 
	    sim$opinion_updating_s <- sim$opinion_updating_s[ , .(sender, receiver, opinion_receiver, assumption_sender, opt_message, op_compl_receiver, msg_compl, actions, best_action)] 
   ...
   if ( (nrow(sim$actions_receive[ best_action == "Heuristic" ]) > 0 && nrow(sim$actions_receive[ best_action == "Systematic" ] > 0 ) ) )  {
	...
	sim$discourse_memory_temp <- copy(sim$s_mode_for_energy)[ , .(agent_id, receiver_business) ] 
	sim$discourse_memory_temp <- rbind(sim$discourse_memory_temp, copy(sim$opinion_updating_h)[ , .(agent_id, receiver_business) ]) 
	...
	sim$discourse_memory[ , receiver_business := ifelse(is.na(receiver_business), 0, receiver_business ) ]  
      ...
      sim$opionion_updating <- unique(sim$opionion_updating) 

      } else {
	   if ( (nrow(sim$actions_receive[ best_action == "Heuristic" ]) > 0 ) ) {

	sim$discourse_memory_temp <- copy(sim$opinion_updating_h)[ , .(agent_id, receiver_business) ] 
	setkey(sim$discourse_memory_temp, "agent_id") 
	sim$discourse_memory_temp <- unique(sim$discourse_memory_temp) 
	sim$discourse_memory <- sim$discourse_memory_temp[sim$discourse_memory[ , -c("receiver_business") ], on=c("agent_id"="sender") ] 
	setnames(sim$discourse_memory, "agent_id", "sender") 
  ...
  sim$opinion_updating[, new_op_compl := op_compl + ifelse(mean(c(unlist(past_op_compls)) - op_compl) > 0, mean(c(unlist(past_op_compls)) - op_compl), 0) ]
...
rc_sh_modelStep <- function(sim) {
  ...
  sim$messages <- rbind(sim$messages_temp, sim$messages) 
  sim$messages <- sim$messages[copy(sim$agent_characteristics)[, .(agent_id, opinion, op_compl)], nomatch = 0L, on = c("sender" = "agent_id")] 
  setnames(sim$messages, "opinion", "opinion_sender")
  ...
  sim$actions_overall_temp <- sim$actions_overall_temp[ !duplicated(sim$actions_overall_temp[ , as.logical(lapply(sim$actions_overall_temp, function(x) !is.list(x) ) ) , with = FALSE ]) , ]
  sim$actions_overall_temp <- sim$actions_overall_temp[copy(sim$discourse_memory)[, .(sender, message)][ , assumption_sender := message][ , -c("message")] %>% unique, on=c("receiver"="sender"), nomatch=0L] 
...
new_series_two <- vector()
  ...
  sim$actions_overall[ actions == "Send" , projected_energy := ifelse(energy_loss <= 0, 0, energy_loss) / ( energy + params(sim)$rc_sh_model$restoration_factor ) ] 
  sim$actions_overall[ actions == "Receive" , projected_energy := ifelse(energy_loss <= 0, 0, energy_loss) / ( energy + params(sim)$rc_sh_model$restoration_factor ) ] 
  sim$actions_overall[ actions == "Both" , projected_energy := ifelse(energy_loss <= 0, 0, energy_loss) / ( energy + params(sim)$rc_sh_model$restoration_factor ) ] 
  ...
  sim$messages_temp <-  unique(sim$discourse_memory[ , .(sender, message) ]) 
  ...
  sim$discourse_memory[ , past_opinions := ifelse(lengths(past_opinions) < params(sim)$rc_sh_model$opinion_memory_depth,
   ...
   if( length(sim$actions_overall[ best_action %in% c("Both", "Send") , best_action ] > 0 ) ) {
    ...
    sim$actions_send_temp <- copy(sim$messages) 
    sim$actions_send_temp <- sim$actions_send_temp[copy(sim$actions_overall)[ best_action == actions ][ (best_action == "Both" | best_action == "Send") ]  , on = c("sender" = "agent_id"), nomatch = 0L ] 
    sim$actions_send_temp <- sim$actions_send_temp[ , .(sender , receiver , opt_message , opinion_sender , assumption_receiver)] 
    ...
    sim$chosen_actions <- merge(sim$actions_send[ actions == best_action , .(agent_id, best_action) ], sim$chosen_actions, by="agent_id", all.x=TRUE , all.y=TRUE) 
    sim$chosen_actions[ best_action.y == "Receive" , action_type := "actions_overall" ] 
    sim$chosen_actions[ best_action.y == "Nothing" , action_type := "actions_overall" ] 
    sim$chosen_actions[ action_type == "actions_send" , best_action := ifelse(!is.na(best_action.x), best_action.x, "NOT ASSIGNED")] 
    sim$chosen_actions[ action_type == "actions_overall" , best_action := ifelse(!is.na(best_action.y), best_action.y, "NOT ASSIGNED")] 
    ...
    sim$send_busy <- unique(copy(sim$actions_send)[ actions==best_action ][ , actions := NULL ][ , -c("util_score")])[sim$messages[ , .(sender, receiver, opt_message) ], on = c("agent_id" = "sender"), nomatch = 0L]  
    ...
    sim$messages <- sim$messages[sim$actions_send[ best_action == actions , .(agent_id, actions, best_action)], nomatch=0L, on=c("sender" = "agent_id")]  
    sim$messages <- unique(sim$messages)
    sim$messages <- merge(sim$messages, sim$discourse_memory[ , .(sender, past_msg_compls, past_op_compls)], nomatch=0L, by=c("sender")) 
    sim$messages <- sim$messages[ !duplicated(sim$messages[ , as.logical(lapply(sim$messages, function(x) !is.list(x) ) ) , with = FALSE ]) , ]
    sim$messages[, msg_compl := mapply(function(a, b, k) {
	...
	sim$chosen_actions <- merge(sim$actions_receive[ actions == best_action , .(agent_id, best_action) ], sim$chosen_actions, by="agent_id", all.x=TRUE , all.y=TRUE) 
	sim$chosen_actions[ best_action.y == "Receive" , action_type := "actions_overall" ] 
	sim$chosen_actions[ best_action.y == "Nothing" , action_type := "actions_overall" ] 
	sim$chosen_actions[ action_type == "actions_send" , best_action := ifelse(!is.na(best_action.y), best_action.y, "NOT ASSIGNED")] 
	sim$chosen_actions[ action_type == "actions_overall" , best_action := ifelse(!is.na(best_action.y), best_action.y, "NOT ASSIGNED")] 
	sim$chosen_actions[ action_type == "actions_receive" , best_action := ifelse(!is.na(best_action.x), best_action.x, "NOT ASSIGNED")] 
        sim$chosen_actions[ , best_action.x := NULL ][ , best_action.y := NULL ]
    ...
    if (nrow(sim$actions_receive[ best_action == "Heuristic" ]) > 0) {
	    ...
	    sim$opinion_updating_h <- copy(sim$messages) 
	    sim$opinion_updating_h <- sim$opinion_updating_h[copy(sim$actions_receive)[ best_action == actions ][ best_action == "Heuristic"] , on = c("receiver" = "agent_id"), nomatch = 0L ] 
	    sim$opinion_updating_h[ , .(sender, receiver, opinion_receiver, assumption_sender, opt_message, op_compl_receiver, msg_compl, actions, best_action)] 
	    sim$opinion_updating_h[ , message_popularity := mapply(function(a,b) {
		sum(
		  sapply(a, function(x) {
	...
	sim$opinion_updating_h <- sim$opinion_updating_h[ unique_within_sender == TRUE ][ , opinion_receiver_new := sum(assumption_sender) / .N, by="receiver" ]  
	setnames(sim$opinion_updating_h, "receiver", "agent_id") 
	sim$opinion_updating_h[ , new_op_compl := 0 ] 
	sim$opinion_updating_h[ , .(agent_id, opinion_receiver_new, new_op_compl)] 
	setkey(sim$opinion_updating_h, "agent_id") 
	sim$opinion_updating_h <- unique(sim$opinion_updating_h)
 ...
 merge_receiver_opinions <- merge_receiver_opinions[ !duplicated(merge_receiver_opinions[ , as.logical(lapply(merge_receiver_opinions, function(x) !is.list(x) ) ) , with = FALSE ]) , ]
    ...
    merge_sender_messages[ !duplicated(merge_sender_messages[ , as.logical(lapply(merge_sender_messages, function(x) !is.list(x) ) ) , with = FALSE ]) , ]

	    sim$opinion_updating_s <- unique(copy(sim$messages)) 
	    sim$opinion_updating_s <- sim$opinion_updating_s[copy(sim$actions_receive)[ best_action == actions ][ best_action == "Systematic"], on = c("receiver" = "agent_id"), nomatch = 0L ]
	    sim$opinion_updating_s <- sim$opinion_updating_s[ , .(sender, receiver, opinion_receiver, assumption_sender, opt_message, op_compl_receiver, msg_compl, actions, best_action)]
            sim$opinion_updating_s <- sim$opinion_updating_s[merge_receiver_opinions, on=c("receiver"="sender"), nomatch = 0L] 
    ...
    sim$opinion_updating_s[ , distance_to_past_messages := mapply(function(a,b) {
	   ...
	   s_mode_for_energy <- s_mode_for_energy[ , receiver_business := .N + sum_msg_compls, by = receiver ] 
   ...
   if ( (nrow(sim$actions_receive[ best_action == "Heuristic" ]) > 0 && nrow(sim$actions_receive[ best_action == "Systematic" ]) > 0 ) )  {
	...
	setkey(sim$discourse_memory_temp, "agent_id") 
	sim$discourse_memory_temp <- unique(sim$discourse_memory_temp) 
	sim$discourse_memory <- sim$discourse_memory_temp[copy(sim$discourse_memory), on=c("agent_id"="sender") ] 
	setnames(sim$discourse_memory, "agent_id", "sender") 
    ...
    if( length(sim$nothingers) > 0  ) {
          ...
          sim$opinion_updating_n <- copy(sim$discourse_memory)[ .(sender, past_opinions, op_compl)][ sender %in% sim$nothingers ] 
          sim$opinion_updating_n[ !duplicated(sim$opinion_updating_n[ , as.logical(lapply(sim$opinion_updating_n, function(x) !is.list(x) ) ) , with = FALSE ]) , ]
